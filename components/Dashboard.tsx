...
import { Wallet, TrendingUp, Filter, Calendar, ArrowUpCircle, ArrowDownCircle } from 'lucide-react';
...
// Custom Palettes
const INCOME_COLORS = ['#84cc16', '#a3e635', '#65a30d', '#bef264', '#4d7c0f'];
const EXPENSE_COLORS = ['#1B365D', '#3b82f6', '#64748b', '#ef4444', '#f59e0b', '#ec4899', '#8b5cf6'];
...
const Dashboard: React.FC<DashboardProps> = ({ transactions, movementTypes }) => {
  ...
  const chartData = useMemo(() => {
    ...
    sorted.forEach(t => {
        let key = filterMode === 'all' ? t.date.substring(0, 7) : t.date;
  ...
  const incomeTypeData = useMemo(() => {
      ...
      currencyTransactions.forEach(t => {
          const type = movementTypes.find(mt => mt.id === t.movementTypeId);
          if (type && type.category === MovementCategory.INCOME) {
  ...
  const expenseTypeData = useMemo(() => {
      ...
      currencyTransactions.forEach(t => {
          const type = movementTypes.find(mt => mt.id === t.movementTypeId);
          if (type && type.category === MovementCategory.EXPENSE) {
              data[type.name] = (data[type.name] || 0) + t.amount; 
          }
      });
      return Object.entries(data)
    ...
    <div className="space-y-6">
      ...
      {!hasData ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(statsByCurrency).map(([currency, data]: [string, any]) => (
      ...
      {/* --- DUAL PIE CHARTS: INCOME VS EXPENSE --- */}
      {hasData && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 min-h-[400px] flex flex-col">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <h3 className="text-lg font-semibold text-[#1B365D]">Distribución de Entradas</h3>
                </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 min-h-[400px] flex flex-col">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <h3 className="text-lg font-semibold text-[#1B365D]">Distribución de Salidas</h3>
                </div>
